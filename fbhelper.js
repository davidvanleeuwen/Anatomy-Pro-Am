(function() {
  var Hash, addMyFriends, addOauthCredental, addUser, addUserAsFriend, associateFriend, authresponse, base64decode, cleanJSON, client, color, config, fbGetFriendsObject, fbGetMeObject, fbgraph, fbutil, formatUser, gatherInitLogin, getFriends, getUser, http, httpClient, https, renderIndex, storeUser, updateOnlineStatus, userDeauthed, userDeclinedAccess;
  ...
  */
  ...
  addMyFriends = function(d, myID) {
    Hash(d.data).forEach(function(friend) {
      ...
      return updateOnlineStatus(myID, friend);
    });
  ...
  updateOnlineStatus = function(UID, friend) {
    var accessToken;
    accessToken = '';
    return getUser(UID, function(cb) {
      var userApps;
      if (cb.error === void 0) {
        userApps = JSON.parse(cb).facebook_user.application_authorizations;
        return Hash(userApps).forEach(function(value, key) {
          var outObject, outdata, path, path1, path2, url;
          if (value.application.app_id === config.fbconfig.appId) {
            ...
            return https.get({
              ...
              res.on('data', function(d) {
                if (JSON.parse(d).error_code === void 0) {
                  if (JSON.parse(d)[0].online_presence === 'active') {
                    outObject['last_online'] = Date.now;
                    return console.log(color(friend.name + " is online", 'green'));
                  }
                }
              });
  ...
  addUser = function(info, callback) {
    return getUser(info.id, function(cb) {
      var dbPath, postData, result;
      console.log('info and callback info');
      console.log(cb, info);
      if (cb.error === 100 && info.first_name === !null) {
        ...
        client.perform(config.sql.fullHost + dbPath, "PUT", function(resp) {
          var result;
          console.log(resp);
          result = resp.response.body;
          return console.log(color('------------------- RESULT OF ADDING USER -------------------\n', 'green'));
        }, postData);
      }
      if (cb.error === 404) {
        ...
        postData = formatUser(info);
        dbPath = '/facebook_users.json';
        client.perform(config.sql.fullHost + dbPath, "POST", function(resp) {
          var result;
          result = resp.response.body;
  ...
  getUser = function(fbid, cb) {
    ...
    return client.perform(config.sql.fullHost + dbPath, "GET", function(res) {
      ...
      if (res.response.status === 404) {
        ...
        if (JSON.parse(res.response.body).facebook_user.first_name === null) {
          result = {
            error: 100
          };
        } else {
