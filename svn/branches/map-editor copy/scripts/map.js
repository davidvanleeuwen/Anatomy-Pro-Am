var TMap = Class.create({
	...
	createTileSpecial: function(sTileUri, iTileWidth, iTileHeight, tempVar) {
		var oTile = this.viewPort.ownerDocument.createElement('div').setStyle({
			'position': 'absolute',
			'width': (iTileWidth + 'px'),
			'height': (iTileHeight + 'px'),
			'background-image': 'url("' + sTileUri + '")',
			'margin-top': (this.tileHeight - iTileHeight-50+tempVar) + 'px',
			'margin-left': (((this.tileWidth - iTileWidth) / 2)-50+tempVar) + 'px',
			'background-repeat': 'no-repeat',
			'background-position': 'center center'
		});
		oTile.tileUrl = sTileUri;
